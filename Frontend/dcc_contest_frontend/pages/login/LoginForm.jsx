...
export default function LoginForm() {
  ...
  const [
    openResendConfirmationEmailModal,
    setOpenResendConfirmationEmailModal,
  ] = useState(false);
  ...
  const onLogin = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        else router.push(`/profile/${res.data.username}`);
      })
      .catch((err) => {
  ...
  return (
    <div className="login-form">
      ...
      <ResendConfirmationEmailModal
        open={openResendConfirmationEmailModal}
        setOpen={setOpenResendConfirmationEmailModal}
      />
      {error && (
      ...
      <div className="flex flex-row gap-2 m-4 justify-between text-green-600">
        ...
        <Link
          href="#"
          onClick={() => setOpenResendConfirmationEmailModal(true)}
        >
          {" "}
          Resend Confirmation
        </Link>
      ...
      <div>
        <Link href="#" onClick={() => router.push(SIGNUP_PAGE)}>
          {" "}
          New to Code-DCC ? Sign Up
        </Link>
      </div>
    </div>
  );
